...
namespace ConsoleApp1
{
    public class ProxyHelp
    {
        ...
        #endregion
        public static List<WebProxy> GetProxyLsit()
        {
            //获取代理集合
            Console.WriteLine("获取代理ip集合");
            ConcurrentQueue<string> proxyIpQueue = new ConcurrentQueue<string>();
            new ProxyHelp().Grab_ProxyIp(proxyIpQueue);
            List<WebProxy> proxyList = new List<WebProxy>();
            foreach (var item in proxyIpQueue)
            {
                var temp = new WebProxy(item.Split(':')[0], Convert.ToInt32(item.Split(':')[1]));
                try
                {
                    ...
                    Console.WriteLine($"测试代理Ip:{temp.Address}");
                    request.Proxy = temp;
            ...
            Console.WriteLine($"获取完毕，共{proxyList.Count}个ip");
            return proxyList;
        }
